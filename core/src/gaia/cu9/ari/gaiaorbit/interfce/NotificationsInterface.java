...
public class NotificationsInterface extends Table implements IObserver, IGuiInterface {
	...
	EventManager.instance.subscribe(this, Events.POST_NOTIFICATION, Events.FOCUS_CHANGED, Events.TOGGLE_TIME_CMD,
		Events.TOGGLE_VISIBILITY_CMD, Events.CAMERA_MODE_CMD, Events.PACE_CHANGED_INFO, Events.FOCUS_LOCK_CMD,
		Events.TOGGLE_AMBIENT_LIGHT, Events.FOV_CHANGE_NOTIFICATION, Events.JAVA_EXCEPTION,
		Events.ORBIT_DATA_LOADED, Events.SCREENSHOT_INFO, Events.COMPUTE_GAIA_SCAN_CMD,
		Events.ONLY_OBSERVED_STARS_CMD, Events.TRANSIT_COLOUR_CMD, Events.LIMIT_MAG_CMD,
		Events.TOGGLE_STEREOSCOPIC_CMD, Events.DISPLAY_GUI_CMD, Events.FRAME_OUTPUT_CMD,
		Events.TOGGLE_STEREO_PROFILE_CMD, Events.OCTREE_PARTICLE_FADE_CMD);
    }

    public void unsubscribe() {
	EventManager.instance.unsubscribe(this, Events.POST_NOTIFICATION, Events.FOCUS_CHANGED, Events.TOGGLE_TIME_CMD,
		Events.TOGGLE_VISIBILITY_CMD, Events.CAMERA_MODE_CMD, Events.PACE_CHANGED_INFO, Events.FOCUS_LOCK_CMD,
		Events.TOGGLE_AMBIENT_LIGHT, Events.FOV_CHANGE_NOTIFICATION, Events.JAVA_EXCEPTION,
		Events.ORBIT_DATA_LOADED, Events.SCREENSHOT_INFO, Events.COMPUTE_GAIA_SCAN_CMD,
		Events.ONLY_OBSERVED_STARS_CMD, Events.TRANSIT_COLOUR_CMD, Events.LIMIT_MAG_CMD,
	...
	synchronized (lock) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (i < data.length - 1
				&& !(i == data.length - 2 && data[data.length - 1] instanceof Boolean)) {
		...
		if (data.length == 3)
		    addMessage(I18n.bundle.format("notif.visibility." + (((Boolean) data[2]) ? "on" : "off"),
			    I18n.bundle.get((String) data[0])));
		else
		...
		if (data.length == 1) {
		    ...
		    addMessage(I18n.bundle.format("notif.error",
			    data[1] + TAG_SEPARATOR + ((Throwable) data[0]).toString()));
		}
		break;
		...
		addMessage(
			I18n.bundle.format("notif.stereoscopic.profile", GlobalConf.program.STEREO_PROFILE.toString()));
		break;
