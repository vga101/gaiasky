...
public class NaturalInputListener extends GestureDetector implements IObserver {
	...
	public NaturalInputListener(final NaturalCamera camera) {
		...
		EventManager.instance.subscribe(this, Events.TOUCH_DOWN, Events.TOUCH_UP, Events.TOUCH_DRAGGED, Events.SCROLLED,
		...
		if (GlobalConf.runtime.INPUT_ENABLED) {
			...
			if (button == this.button && button == Input.Buttons.LEFT) {
				...
				Gdx.app.postRunnable(() -> {
					...
					if (!GlobalConf.scene.CINEMATIC_CAMERA
							|| (GlobalConf.scene.CINEMATIC_CAMERA && gesture.dst(screenX, screenY) < MOVE_PX_DIST)) {
