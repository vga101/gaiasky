...
public class KeyBindings {
	...
	addMapping(new ProgramAction(txt("action.screenshot"), new Runnable() {
	    @Override
	    public void run() {
		EventManager.instance.post(Events.SCREENSHOT_CMD, GlobalConf.screenshot.SCREENSHOT_WIDTH,
			GlobalConf.screenshot.SCREENSHOT_HEIGHT, GlobalConf.screenshot.SCREENSHOT_FOLDER);
	    }
	}), Keys.F5);
	...
	addMapping(new ProgramAction(txt("action.toggle", txt("element.octreeparticlefade")), new Runnable() {
	    @Override
	    public void run() {
		EventManager.instance.post(Events.OCTREE_PARTICLE_FADE_CMD, txt("element.octreeparticlefade"),
			!GlobalConf.scene.OCTREE_PARTICLE_FADE);
	    }
