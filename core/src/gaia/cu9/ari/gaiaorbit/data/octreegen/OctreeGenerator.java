...
public class OctreeGenerator {
    ...
    public OctreeNode generateOctree(Array<Particle> catalog) {
	...
	if (SUN_CENTRE) {
	    ...
	    root = new OctreeNode(nextPageId(), box.getCenterX(), box.getCenterY(), box.getCenterZ(), halfSize,
		    halfSize, halfSize, 0);
	}
	...
	treatLevel(inputLists, 0, octantsPerLevel,
		MathUtils.clamp((float) aggregation.getMaxPart() / (float) catalog.size, 0f, 1f));
     ...
     * @param percentage
     */
    private void treatLevel(Map<OctreeNode, Array<Particle>> inputLists, int level, List<OctreeNode>[] octantsPerLevel,
	    float percentage) {
	...
	for (OctreeNode octant : levelOctants) {
		...
		/** CREATE SUB-OCTANTS **/
		// Front - top - left
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x - hsx,
			octant.centre.y + hsy, octant.centre.z - hsz, hsx, hsy, hsz, octant.depth + 1, octant, 0));
		// Front - top - right
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x + hsx,
			octant.centre.y + hsy, octant.centre.z - hsz, hsx, hsy, hsz, octant.depth + 1, octant, 1));
		// Front - bottom - left
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x - hsx,
			octant.centre.y - hsy, octant.centre.z - hsz, hsx, hsy, hsz, octant.depth + 1, octant, 2));
		// Front - bottom - right
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x + hsx,
			octant.centre.y - hsy, octant.centre.z - hsz, hsx, hsy, hsz, octant.depth + 1, octant, 3));
		// Back - top - left
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x - hsx,
			octant.centre.y + hsy, octant.centre.z + hsz, hsx, hsy, hsz, octant.depth + 1, octant, 4));
		// Back - top - right
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x + hsx,
			octant.centre.y + hsy, octant.centre.z + hsz, hsx, hsy, hsz, octant.depth + 1, octant, 5));
		// Back - bottom - left
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x - hsx,
			octant.centre.y - hsy, octant.centre.z + hsz, hsx, hsy, hsz, octant.depth + 1, octant, 6));
		// Back - bottom - right
		octantsPerLevel[level + 1].add(new OctreeNode(nextPageId(), octant.centre.x + hsx,
			octant.centre.y - hsy, octant.centre.z + hsz, hsx, hsy, hsz, octant.depth + 1, octant, 7));
	    }
	}
	...
	if (!octantsPerLevel[level + 1].isEmpty()) {
	    ...
	    float sublevelPercentage = MathUtils.clamp((float) aggregation.getMaxPart() / (float) maxSublevelObjs, 0f,
		    1f);
     ...
     * @return
     */
    private Array<Particle> intersect(Array<Particle> stars, OctreeNode box) {
     ...
     * @param percentage
     */
    private void treatOctant(OctreeNode octant, Array<Particle> catalog, float percentage) {
	...
	if (!leaf) {
	    ...
	    // Front - top - left
	    nodes[0] = new OctreeNode(nextPageId(), octant.centre.x - hsx, octant.centre.y + hsy, octant.centre.z - hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Front - top - right
	    nodes[1] = new OctreeNode(nextPageId(), octant.centre.x + hsx, octant.centre.y + hsy, octant.centre.z - hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Front - bottom - left
	    nodes[2] = new OctreeNode(nextPageId(), octant.centre.x - hsx, octant.centre.y - hsy, octant.centre.z - hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Front - bottom - right
	    nodes[3] = new OctreeNode(nextPageId(), octant.centre.x + hsx, octant.centre.y - hsy, octant.centre.z - hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Back - top - left
	    nodes[4] = new OctreeNode(nextPageId(), octant.centre.x - hsx, octant.centre.y + hsy, octant.centre.z + hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Back - top - right
	    nodes[5] = new OctreeNode(nextPageId(), octant.centre.x + hsx, octant.centre.y + hsy, octant.centre.z + hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Back - bottom - left
	    nodes[6] = new OctreeNode(nextPageId(), octant.centre.x - hsx, octant.centre.y - hsy, octant.centre.z + hsz,
		    hsx, hsy, hsz, octant.depth + 1);
	    // Back - bottom - right
	    nodes[7] = new OctreeNode(nextPageId(), octant.centre.x + hsx, octant.centre.y - hsy, octant.centre.z + hsz,
		    hsx, hsy, hsz, octant.depth + 1);
