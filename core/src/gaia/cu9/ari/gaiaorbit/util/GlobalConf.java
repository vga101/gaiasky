...
public class GlobalConf {
	...
	public void initialize(int sCREENSHOT_WIDTH, int sCREENSHOT_HEIGHT, String sCREENSHOT_FOLDER,
	...
	public PostprocessConf() {
	    EventManager.instance.subscribe(this, Events.BLOOM_CMD, Events.LENS_FLARE_CMD, Events.MOTION_BLUR_CMD,
		    Events.LIGHT_SCATTERING_CMD, Events.FISHEYE_CMD, Events.BRIGHTNESS_CMD, Events.CONTRAST_CMD);
	}

	public void initialize(int POSTPROCESS_ANTIALIAS, float POSTPROCESS_BLOOM_INTENSITY,
		float POSTPROCESS_MOTION_BLUR, boolean POSTPROCESS_LENS_FLARE, boolean POSTPROCESS_LIGHT_SCATTERING,
	...
	public void notify(Events event, Object... data) {
		...
		break;
	    case BRIGHTNESS_CMD:
		POSTPROCESS_BRIGHTNESS = MathUtils.clamp((float) data[0], Constants.MIN_BRIGHTNESS,
	...
	public RuntimeConf() {
	    EventManager.instance.subscribe(this, Events.LIMIT_MAG_CMD, Events.INPUT_ENABLED_CMD,
		    Events.DISPLAY_GUI_CMD, Events.TOGGLE_UPDATEPAUSE, Events.TOGGLE_TIME_CMD,
		    Events.RECORD_CAMERA_CMD);
	}

	public void initialize(boolean dISPLAY_GUI, boolean uPDATE_PAUSE, boolean sTRIPPED_FOV_MODE, boolean tIME_ON,
		boolean iNPUT_ENABLED, boolean rECORD_CAMERA, float lIMIT_MAG_RUNTIME, boolean rEAL_TIME,
		boolean dRAW_OCTREE) {
	...
	public void initialize(int rENDER_WIDTH, int rENDER_HEIGHT, int rENDER_TARGET_FPS, int cAMERA_REC_TARGET_FPS,
		boolean aUTO_FRAME_OUTPUT_CAMERA_PLAY, String rENDER_FOLDER, String rENDER_FILE_NAME,
		boolean rENDER_SCREENSHOT_TIME, boolean rENDER_OUTPUT, ScreenshotMode fRAME_MODE) {
	...
	public void initialize(boolean oBJECT_SERVER_CONNECTION, String cATALOG_JSON_FILE, String hYG_JSON_FILE,
		String tGAS_JSON_FILE, String dR2_JSON_FILE, String oBJECTS_JSON_FILE, String[] oBJECTS_JSON_FILE_GQ,
		String oBJECT_SERVER_HOSTNAME, int oBJECT_SERVER_PORT, String vISUALIZATION_ID, float lIMIT_MAG_LOAD,
	...
	public void initialize(String cATALOG_JSON_FILE, String oBJECTS_JSON_FILE, boolean dATA_SOURCE_LOCAL,
	...
	public void initialize(String cATALOG_JSON_FILE, String dATA_JSON_FILE, boolean dATA_SOURCE_LOCAL,
		float lIMIT_MAG_LOAD, boolean rEAL_GAIA_ATTITUDE) {
	    this.CATALOG_JSON_FILE = cATALOG_JSON_FILE;
	...
	public void initialize(int sCREEN_WIDTH, int sCREEN_HEIGHT, int fULLSCREEN_WIDTH, int fULLSCREEN_HEIGHT,
		boolean fULLSCREEN, boolean rESIZABLE, boolean vSYNC, boolean sCREEN_OUTPUT) {
	...
	public void initialize(boolean dISPLAY_TUTORIAL, String tUTORIAL_POINTER_SCRIPT_LOCATION,
	...
	public void initialize(boolean dISPLAY_TUTORIAL, boolean sHOW_DEBUG_INFO, String uI_THEME, String lOCALE,
		boolean sTEREOSCOPIC_MODE, StereoProfile sTEREO_PROFILE) {
	    DISPLAY_TUTORIAL = dISPLAY_TUTORIAL;
	    SHOW_DEBUG_INFO = sHOW_DEBUG_INFO;
	    UI_THEME = uI_THEME;
	    LOCALE = lOCALE;
	...
	public boolean isStereoHalfWidth() {
	    return STEREOSCOPIC_MODE
		    && (STEREO_PROFILE != StereoProfile.HD_3DTV && STEREO_PROFILE != StereoProfile.ANAGLYPHIC);
	}
	...
	public void notify(Events event, Object... data) {
		...
		if (!GaiaSky.instance.cam.mode.isGaiaFov()) {
		    ...
		    EventManager.instance.post(Events.POST_NOTIFICATION,
			    "You have entered 3D mode. Go back to normal mode using <CTRL+S>");
		    EventManager.instance.post(Events.POST_NOTIFICATION,
			    "Switch between stereoscopic modes using <CTRL+SHIFT+S>");
		}
		break;
		...
		EventManager.instance.post(Events.DISPLAY_GUI_CMD, I18n.bundle.get("notif.cleanmode"),
			!CUBEMAP360_MODE);
		...
		// }
		EventManager.instance.post(Events.POST_NOTIFICATION,
			"You have entered the 360 mode.  Go back to normal mode using <CTRL+3>");
		break;
	    default:
		break;
	    }
	...
	public void initialize(String version, String buildtime, String builder, String system, String build, int major,
		int minor, int rev) {
	...
	public static int[] getMajorMinorRevFromString(String version) {
	    ...
	    return new int[] { Integer.parseInt(majorS), Integer.parseInt(minorS),
		    revS != null ? Integer.parseInt(revS) : 0 };
	}
	...
	public SceneConf() {
	    EventManager.instance.subscribe(this, Events.TOGGLE_VISIBILITY_CMD, Events.FOCUS_LOCK_CMD,
		    Events.ORIENTATION_LOCK_CMD, Events.PROPER_MOTIONS_CMD, Events.STAR_BRIGHTNESS_CMD,
		    Events.PM_LEN_FACTOR_CMD, Events.PM_NUM_FACTOR_CMD, Events.FOV_CHANGED_CMD, Events.CAMERA_SPEED_CMD,
		    Events.ROTATION_SPEED_CMD, Events.TURNING_SPEED_CMD, Events.SPEED_LIMIT_CMD,
		    Events.TRANSIT_COLOUR_CMD, Events.ONLY_OBSERVED_STARS_CMD, Events.COMPUTE_GAIA_SCAN_CMD,
		    Events.PIXEL_RENDERER_CMD, Events.OCTREE_PARTICLE_FADE_CMD, Events.STAR_POINT_SIZE_CMD,
		    Events.STAR_POINT_SIZE_INCREASE_CMD, Events.STAR_POINT_SIZE_DECREASE_CMD,
		    Events.STAR_POINT_SIZE_RESET_CMD, Events.STAR_MIN_OPACITY_CMD, Events.AMBIENT_LIGHT_CMD,
		    Events.GALAXY_3D_CMD, Events.CROSSHAIR_CMD);
	}

	public void initialize(int gRAPHICS_QUALITY, long oBJECT_FADE_MS, float sTAR_BRIGHTNESS, float aMBIENT_LIGHT,
		int cAMERA_FOV, float cAMERA_SPEED, float tURNING_SPEED, float rOTATION_SPEED,
		int cAMERA_SPEED_LIMIT_IDX, boolean fOCUS_LOCK, boolean fOCUS_LOCK_ORIENTATION,
		float lABEL_NUMBER_FACTOR, boolean[] vISIBILITY, int pIXEL_RENDERER, int lINE_RENDERER,
		double sTAR_TH_ANGLE_NONE, double sTAR_TH_ANGLE_POINT, double sTAR_TH_ANGLE_QUAD, float pOINT_ALPHA_MIN,
	...
	public void notify(Events event, Object... data) {
		...
		PM_NUM_FACTOR = Math.min(Constants.MAX_PM_NUM_FACTOR,
			Math.max(Constants.MIN_PM_NUM_FACTOR, (float) data[0]));
		break;
	    case PM_LEN_FACTOR_CMD:
		PM_LEN_FACTOR = Math.min(Constants.MAX_PM_LEN_FACTOR,
			Math.max(Constants.MIN_PM_LEN_FACTOR, (float) data[0]));
		break;
		...
		STAR_POINT_SIZE = Math.min(STAR_POINT_SIZE + Constants.STEP_STAR_POINT_SIZE,
		...
		STAR_POINT_SIZE = Math.max(STAR_POINT_SIZE - Constants.STEP_STAR_POINT_SIZE,
     ...
     */
    public static void initialize(VersionConf vc, ProgramConf pc, SceneConf sc, DataConf dc, RuntimeConf rc,
	    PostprocessConf ppc, PerformanceConf pfc, FrameConf fc, ScreenConf scrc, ScreenshotConf shc,
	    ControlsConf cc) throws Exception {
	if (!initialized) {
