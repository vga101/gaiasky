...
public class AstroUtils {
	...
	double frac = (1d / 12d) * month + (1d / 365.242d) * day + (1d / 8765.81d) * hour + (1d / 525949d) * min
		+ (1d / 31556940d) * sec + (1d / 31556940e9d) * nanos;
	...
	double lonecl = 238.9508 + 0.00400703 * d - 19.799 * FastMath.sin(P) + 19.848 * FastMath.cos(P)
		+ 0.897 * FastMath.sin(2 * P) - 4.956 * FastMath.cos(2 * P) + 0.610 * FastMath.sin(3 * P)
		+ 1.211 * FastMath.cos(3 * P) - 0.341 * FastMath.sin(4 * P) - 0.190 * FastMath.cos(4 * P)
		+ 0.128 * FastMath.sin(5 * P) - 0.034 * FastMath.cos(5 * P) - 0.038 * FastMath.sin(6 * P)
		+ 0.031 * FastMath.cos(6 * P) + 0.020 * FastMath.sin(S - P) - 0.010 * FastMath.cos(S - P);

	double latecl = -3.9082 - 5.453 * FastMath.sin(P) - 14.975 * FastMath.cos(P) + 3.527 * FastMath.sin(2 * P)
		+ 1.673 * FastMath.cos(2 * P) - 1.051 * FastMath.sin(3 * P) + 0.328 * FastMath.cos(3 * P)
		+ 0.179 * FastMath.sin(4 * P) - 0.292 * FastMath.cos(4 * P) + 0.019 * FastMath.sin(5 * P)
		+ 0.100 * FastMath.cos(5 * P) - 0.031 * FastMath.sin(6 * P) - 0.026 * FastMath.cos(6 * P)
		+ 0.011 * FastMath.cos(S - P);

	double r = 40.72 + 6.68 * FastMath.sin(P) + 6.90 * FastMath.cos(P) - 1.18 * FastMath.sin(2 * P)
		- 0.03 * FastMath.cos(2 * P) + 0.15 * FastMath.sin(3 * P) - 0.14 * FastMath.cos(3 * P);
     ...
     * @return
     */
    private static double[] calculateSumlSumr(double D, double M, double Mp, double F, double E, double A1, double A2,
	    double Lp) {
	...
	for (int i = 0; i < table45a.length; i++) {
	    ...
	    suml += curr[4] * FastMath.sin(Math.toRadians(curr[0] * D + curr[1] * M + curr[2] * Mp + curr[3] * F))
		    * mul;
	    sumr += curr[5] * FastMath.cos(Math.toRadians(curr[0] * D + curr[1] * M + curr[2] * Mp + curr[3] * F))
		    * mul;
	}
	...
	double sumladd = 3958 * FastMath.sin(Math.toRadians(A1)) + 1962 * FastMath.sin(Math.toRadians(Lp - F))
		+ 318 * FastMath.sin(Math.toRadians(A2));
	suml += sumladd;
    ...
    private static double calculateSumb(double D, double M, double Mp, double F, double E, double A1, double A3,
	    double Lp) {
	double sumb = 0;
	for (int i = 0; i < table45b.length; i++) {
	    ...
	    sumb += curr[4] * FastMath.sin(Math.toRadians(curr[0] * D + curr[1] * M + curr[2] * Mp + curr[3] * F))
		    * mul;
	}
	// Addition to Sumb. The terms involving A1 are due to the action of
	// Venus. The term involving A2 is due to Jupiter
	// while those involing L' are due to the flattening of the Earth.
	double sumbadd = -2235 * FastMath.sin(Math.toRadians(Lp)) + 382 * FastMath.sin(Math.toRadians(A3))
		+ 175 * FastMath.sin(Math.toRadians(A1 - F)) + 175 * FastMath.sin(Math.toRadians(A1 + F))
		+ 127 * FastMath.sin(Math.toRadians(Lp - Mp)) - 115 * FastMath.sin(Math.toRadians(Lp + Mp));
	sumb += sumbadd;
     ...
     */
    private static final int[][] table45a = { { 0, 0, 1, 0, 6288774, -20905355 }, { 2, 0, -1, 0, 1274027, -3699111 },
	    { 2, 0, 0, 0, 658314, -2955968 }, { 0, 0, 2, 0, 213618, -569925 }, { 0, 1, 0, 0, -185116, 48888 },
	    { 0, 0, 0, 2, -114332, -3149 }, { 2, 0, -2, 0, 58793, 246158 }, { 2, -1, -1, 0, 57066, -152138 },
	    { 2, 0, 1, 0, 53322, -170733 }, { 2, -1, 0, 0, 45758, -204586 }, { 0, 1, -1, 0, -40923, -129620 },
	    { 1, 0, 0, 0, -34720, 108743 }, { 0, 1, 1, 0, -30383, 104755 }, { 2, 0, 0, -2, 15327, 10321 },
	    { 0, 0, 1, 2, -12528, 0 }, { 0, 0, 1, -2, 10980, 79661 }, { 4, 0, -1, 0, 10675, -34782 },
	    { 0, 0, 3, 0, 10034, -23210 }, { 4, 0, -2, 0, 8548, -21636 }, { 2, 1, -1, 0, -7888, 24208 },
	    { 2, 1, 0, 0, -6766, 30824 }, { 1, 0, -1, 0, -5163, -8379 }, { 1, 1, 0, 0, 4987, -16675 },
	    { 2, -1, 1, 0, 4036, -12831 }, { 2, 0, 2, 0, 3994, -10445 }, { 4, 0, 0, 0, 3861, -11650 },
	    { 2, 0, -3, 0, 3665, 14403 }, { 0, 1, -2, 0, -2689, -7003 }, { 2, 0, -1, 2, -2602, 0 },
	    { 2, -1, -2, 0, 2390, 10056 }, { 1, 0, 1, 0, -2348, 6322 }, { 2, -2, 0, 0, 2236, -9884 },
	    { 0, 1, 2, 0, -2120, 5751 }, { 0, 2, 0, 0, -2069, 0 }, { 2, -2, -1, 0, 2048, -4950 },
	    { 2, 0, 1, -2, -1773, 4130 }, { 2, 0, 0, 2, -1595, 0 }, { 4, -1, -1, 0, 1215, -3958 },
	    { 0, 0, 2, 2, -1110, 0 }, { 3, 0, -1, 0, -892, 3258 }, { 2, 1, 1, 0, -810, 2616 },
	    { 4, -1, -2, 0, 759, -1897 }, { 0, 2, -1, 0, -713, -2117 }, { 2, 2, -1, 0, -700, 2354 },
     ...
     * argument
     */
    private static final int[][] table45b = { { 0, 0, 0, 1, 5128122, 0 }, { 0, 0, 1, 1, 280602, 0 },
	    { 0, 0, 1, -1, 277693, 0 }, { 2, 0, 0, -1, 173237, 0 }, { 2, 0, -1, 1, 55413, 0 },
	    { 2, 0, -1, -1, 46271, 0 }, { 2, 0, 0, 1, 32573, 0 }, { 0, 0, 2, 1, 17198, 0 }, { 2, 0, 1, -1, 9266, 0 },
	    { 0, 0, 2, -1, 8822, 0 }, { 2, -1, 0, -1, 8216, 0 }, { 2, 0, -2, -1, 4324, 0 }, { 2, 0, 1, 1, 4200, 0 },
	    { 2, 1, 0, -1, -3359, 0 }, { 2, -1, -1, 1, 2463, 0 }, { 2, -1, 0, 1, 2211, 0 }, { 2, -1, -1, -1, 2065, 0 },
	    { 0, 1, -1, -1, -1870, 0 }, { 4, 0, -1, -1, 1828, 0 }, { 0, 1, 0, 1, -1794, 0 }, { 0, 0, 0, 3, -1749, 0 },
	    { 0, 1, -1, 1, -1565, 0 }, { 1, 0, 0, 1, -1491, 0 }, { 0, 1, 1, 1, -1475, 0 }, { 0, 1, 1, -1, -1410, 0 },
	    { 0, 1, 0, -1, -1344, 0 }, { 1, 0, 0, -1, -1335, 0 }, { 0, 0, 3, 1, 1107, 0 }, { 4, 0, 0, -1, 1021, 0 },
	    { 4, 0, -1, 1, 833, 0 }, { 0, 0, 1, -3, 777, 0 }, { 4, 0, -2, 1, 671, 0 }, { 2, 0, 0, -3, 607, 0 },
	    { 2, 0, 2, -1, 596, 0 }, { 2, -1, 1, -1, 491, 0 }, { 2, 0, -2, 1, -451, 0 }, { 0, 0, 3, -1, 439, 0 },
	    { 2, 0, 2, 1, 422, 0 }, { 2, 0, -3, -1, 421, 0 }, { 2, 1, -1, 1, -366, 0 }, { 2, 1, 0, 1, -351, 0 },
	    { 4, 0, 0, 1, 331, 0 }, { 2, -1, 1, 1, 315, 0 }, { 2, -2, 0, -1, 302, 0 }, { 0, 0, 1, 3, -283, 0 },
	    { 2, 1, 1, -1, -229, 0 }, { 1, 1, 0, -1, 223, 0 }, { 1, 1, 0, 1, 223, 0 }, { 0, 1, -2, -1, -220, 0 },
	    { 2, 1, -1, -1, -220, 0 }, { 1, 0, 1, 1, -185, 0 }, { 2, -1, -2, -1, 181, 0 }, { 0, 1, 2, 1, -177, 0 },
	    { 4, 0, -2, -1, 176, 0 }, { 4, -1, -1, -1, 166, 0 }, { 1, 0, 1, -1, -164, 0 }, { 4, 0, 1, -1, 132, 0 },
     ...
     * @return
     */
    public static double getJulianDate(int year, int month, int day, int hour, int min, int sec, int nanos,
	    boolean gregorian) {
	if (gregorian) {
	    return getJulianDayNumberWikipediaGregorianCalendar(year, month, day)
		    + getDayFraction(hour, min, sec, nanos);
	} else {
	...
	return 23.0 + 26.0 / 60.0
		+ (21.406 - (46.836769 - (0.0001831 + (0.00200340 - (0.576e-6 - 4.34e-8 * T) * T) * T) * T) * T)
			/ 3600.0;
    }
