...
public class OctreeNode implements ILineRenderable {
	...
	str.append(objects != null ? objects.size : "0").append("/").append(ownObjects).append(", recobj: ")
		.append(nObjects).append(", nchld: ").append(childrenCount).append("] ").append(status).append("\n");
	...
	if (observed) {
		...
		if (GlobalConf.scene.OCTREE_PARTICLE_FADE) {
		    ...
		    alpha = MathUtilsd
			    .clamp(MathUtilsd.lint(viewAngle, GlobalConf.scene.OCTANT_THRESHOLD_0 / cam.getFovFactor(),
				    GlobalConf.scene.OCTANT_THRESHOLD_1 / cam.getFovFactor(), 0d, 1d), 0f, 1f);
		}
	...
	observed = GlobalConf.program.CUBEMAP360_MODE || frustum.pointInFrustum(boxcopy.getCenter(auxD1))
	...
	for (int i = 0; i < 4; i++) {
		...
		observed = Intersectord.intersectRayBoundsFast(ray, boxcopy.getCenter(auxD3),
			boxcopy.getDimensions(auxD4));
	    } else {
		break;
	    }
	}
	...
	this.col.set(ColourUtils.getRed(rgb) * alpha, ColourUtils.getGreen(rgb) * alpha,
		ColourUtils.getBlue(rgb) * alpha, alpha * opacity);
