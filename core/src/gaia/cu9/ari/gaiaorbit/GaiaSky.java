...
public class GaiaSky implements ApplicationListener, IObserver, IMainRenderer {
	...
	manager.load(ATTITUDE_FOLDER, GaiaAttitudeServer.class,
		new GaiaAttitudeLoaderParameter(GlobalConf.runtime.STRIPPED_FOV_MODE
	...
	if (sg == null) {
	    ...
	    manager.load(dataLoadString, ISceneGraph.class, new SGLoaderParameter(time,
		    GlobalConf.performance.MULTITHREADING, GlobalConf.performance.NUMBER_THREADS()));
	}
	...
	Logger.info(this.getClass().getSimpleName(),
		I18n.bundle.format("notif.glslversion", Gdx.gl.glGetString(GL20.GL_SHADING_LANGUAGE_VERSION)));
    }
	...
	Timer.schedule(new Task() {
		...
		EventManager.instance.post(Events.DEBUG2, MemInfo.getUsedMemory(), MemInfo.getFreeMemory(),
			MemInfo.getTotalMemory(), MemInfo.getMaxMemory());
		EventManager.instance.post(Events.DEBUG4,
			GLFrameBuffer.getManagedStatus() + ", Observed octants: " + OctreeNode.nObserved);
	    }
	...
	if (LOADING) {
		...
		if (GlobalConf.screen.SCREEN_OUTPUT) {
		    /** RENDER THE SCENE **/
		    preRenderScene();
		    renderSgr(cam, t, Gdx.graphics.getWidth(), Gdx.graphics.getHeight(), null,
			    pp.getPostProcessBean(RenderType.screen));
    ...
    public void renderSgr(ICamera camera, float t, int width, int height, FrameBuffer frameBuffer,
	    PostProcessBean ppb) {
    ...
    public void resizeImmediate(final int width, final int height, boolean resizePostProcessors,
