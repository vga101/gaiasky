...
public class QuadRenderSystem extends AbstractRenderSystem implements IObserver {
     ...
     */
    public QuadRenderSystem(RenderGroup rg, int priority, float[] alphas, ShaderProgram shaderProgram,
	    boolean useStarColorTransit, String tex) {
    ...
    private void init(String tex) {
	if (tex == null) {
	    if (GlobalConf.scene.isHighQuality()) {
		texture = new Texture(Gdx.files.internal("img/star_glow.png"), true);
	    } else if (GlobalConf.scene.isNormalQuality()) {
		texture = new Texture(Gdx.files.internal("img/star_glow_s.png"), true);
	    } else {
		texture = new Texture(Gdx.files.internal("img/star_glow_s.png"), true);
	    }
	} else {
	    texture = new Texture(Gdx.files.internal(tex), true);
	}
	...
	mesh = new Mesh(VertexDataType.VertexArray, true, 4, 6,
