...
public class StarGroupRenderSystem extends ImmediateRenderSystem implements IObserver {
	...
	protected void initShaderProgram() {
		pointAlpha = new float[] { GlobalConf.scene.POINT_ALPHA_MIN,
				GlobalConf.scene.POINT_ALPHA_MIN + GlobalConf.scene.POINT_ALPHA_MAX };
	}
	...
	private int addMeshData(int nVertices) {
		...
		pmOffset = curr.mesh.getVertexAttribute(Usage.Tangent) != null
				...
				: 0;
		sizeOffset = curr.mesh.getVertexAttribute(Usage.Generic) != null
					...
					if (!starGroup.disposed) {
						...
						if (!starGroup.inGpu) {
							...
							for (StarBean p : starGroup.data()) {
								...
								// SIZE
								curr.vertices[curr.vertexIdx
										+ sizeOffset] = (float) (p.size() * Constants.STAR_SIZE_FACTOR);
						...
						if (curr != null) {
							...
							alphaSizeFovBr[1] = fovmode == 0
									? (GlobalConf.scene.STAR_POINT_SIZE * rc.scaleFactor
											* (GlobalConf.program.isStereoFullWidth() ? 1 : 2))
									: (GlobalConf.scene.STAR_POINT_SIZE * rc.scaleFactor * 10);
							...
							// Days since epoch
							shaderProgram.setUniformi("u_t",
									(int) (AstroUtils.getMsSince(GaiaSky.instance.time.getTime(), starGroup.getEpoch())
											* Constants.MS_TO_D));
