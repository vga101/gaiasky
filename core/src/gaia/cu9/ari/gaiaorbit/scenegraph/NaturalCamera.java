...
public class NaturalCamera extends AbstractCamera implements IObserver {
	...
	public void initialize(AssetManager assetManager) {
		...
		camLeft = new PerspectiveCamera(GlobalConf.scene.CAMERA_FOV, Gdx.graphics.getWidth() / 2,
				Gdx.graphics.getHeight());
		camLeft.near = (float) CAM_NEAR;
		camLeft.far = (float) CAM_FAR;

		camRight = new PerspectiveCamera(GlobalConf.scene.CAMERA_FOV, Gdx.graphics.getWidth() / 2,
				Gdx.graphics.getHeight());
		...
		EventManager.instance.subscribe(this, Events.FOCUS_CHANGE_CMD, Events.FOV_CHANGED_CMD, Events.FOCUS_LOCK_CMD,
				Events.CAMERA_POS_CMD, Events.CAMERA_DIR_CMD, Events.CAMERA_UP_CMD, Events.CAMERA_FWD,
				Events.CAMERA_ROTATE, Events.CAMERA_PAN, Events.CAMERA_ROLL, Events.CAMERA_TURN, Events.CAMERA_STOP,
				Events.CAMERA_CENTER, Events.GO_TO_OBJECT_CMD);
	}
	...
	private void camUpdate(float dt, ITimeFrameProvider time) {
		...
		case Focus:
			if (focus.withinMagLimit()) {
				...
				EventManager.instance.post(Events.FOCUS_INFO_UPDATED, focus.distToCamera - focus.getRadius(),
						((AbstractPositionEntity) focus).viewAngle);
			} else {
	 ...
	 * 
	 * @param target
	 *            The target point of the direction vector
	 */
