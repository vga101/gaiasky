...
public abstract class ModelBody extends CelestialBody {
	...
	if (mc != null) {
		...
		mc.dlight.direction.add((float) camera.getPos().x, (float) camera.getPos().y,
	...
	if (sizeFactor != 1 || forceUpdate) {
	    ...
	    localTransform.idt().translate(trnsltn[0], trnsltn[1], trnsltn[2]).scl(size * sizeFactor)
		    .rotate(0, 1, 0, (float) rc.ascendingNode).rotate(0, 0, 1, (float) (rc.inclination + rc.axialTilt))
		    .rotate(0, 1, 0, (float) rc.angle);
	    orientation.idt().rotate(0, 1, 0, (float) rc.ascendingNode).rotate(0, 0, 1,
		    (float) (rc.inclination + rc.axialTilt));
	} else {
