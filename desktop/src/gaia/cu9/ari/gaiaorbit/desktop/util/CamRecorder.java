...
public class CamRecorder implements IObserver {
	...
	EventManager.instance.subscribe(this, Events.RECORD_CAMERA_CMD, Events.PLAY_CAMERA_CMD,
		Events.UPDATE_CAM_RECORDER, Events.STOP_CAMERA_PLAY);
	...
	case RECORDING:
		...
		try {
		    ...
		    os.append(Double.toString(position.x)).append(sep).append(Double.toString(position.y)).append(sep)
			    .append(Double.toString(position.z));
		    os.append(sep).append(Double.toString(direction.x)).append(sep).append(Double.toString(direction.y))
			    .append(sep).append(Double.toString(direction.z));
		    os.append(sep).append(Double.toString(up.x)).append(sep).append(Double.toString(up.y)).append(sep)
			    .append(Double.toString(up.z));
		    os.append("\n");
		} catch (IOException e) {
		    Logger.error(e);
		}
	...
	case PLAYING:
		...
		try {
			...
			dir.set(Parser.parseDouble(tokens[4]), Parser.parseDouble(tokens[5]),
				Parser.parseDouble(tokens[6]));
			upp.set(Parser.parseDouble(tokens[7]), Parser.parseDouble(tokens[8]),
				Parser.parseDouble(tokens[9]));

			position.set(Parser.parseDouble(tokens[1]), Parser.parseDouble(tokens[2]),
	...
	case RECORD_CAMERA_CMD:
		...
		// Annotate by date
		f = new File(SysUtilsFactory.getSysUtils().getDefaultCameraDir(),
			df.format(new Date()) + "_gscamera.dat");
		if (f.exists()) {
		    f.delete();
		}
		try {
