...
public class GaiaSkyDesktop implements IObserver {
	...
	super();
	EventManager.instance.subscribe(this, Events.SHOW_ABOUT_ACTION, Events.SHOW_RUNSCRIPT_ACTION,
		Events.JAVA_EXCEPTION, Events.SHOW_PLAYCAMERA_ACTION, Events.DISPLAY_MEM_INFO_WINDOW);
    }
	...
	if (GlobalConf.screen.FULLSCREEN) {
		...
		if (mode.height == GlobalConf.screen.FULLSCREEN_HEIGHT
			&& mode.width == GlobalConf.screen.FULLSCREEN_WIDTH) {
		    mymode = mode;
		    break;
		}
	    }
	...
	cfg.setBackBufferConfig(8, 8, 8, 8, 24, 0,
		MathUtilsd.clamp(GlobalConf.postprocess.POSTPROCESS_ANTIALIAS, 0, 16));
	cfg.setIdleFPS(0);
	...
	Logger.info("Display mode set to " + Lwjgl3ApplicationConfiguration.getDisplayMode().width + "x"
		+ Lwjgl3ApplicationConfiguration.getDisplayMode().height + ", fullscreen: "
	...
	if (ow || !userFolderConfFile.exists()) {
		...
		copyFile(new File("../android/assets/conf" + File.separator + "global.properties"), userFolderConfFile,
			ow);
	    }
	}
