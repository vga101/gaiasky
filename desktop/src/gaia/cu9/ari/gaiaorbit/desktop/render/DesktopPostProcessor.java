...
public class DesktopPostProcessor implements IPostProcessor, IObserver {
	...
	if (GlobalConf.screenshot.isRedrawMode())
	    pps[RenderType.screenshot.index] = newPostProcessor(getWidth(RenderType.screenshot),
		    getHeight(RenderType.screenshot));
	...
	EventManager.instance.subscribe(this, Events.SCREENSHOT_SIZE_UDPATE, Events.FRAME_SIZE_UDPATE, Events.BLOOM_CMD,
		Events.LENS_FLARE_CMD, Events.MOTION_BLUR_CMD, Events.LIGHT_POS_2D_UPDATED, Events.LIGHT_SCATTERING_CMD,
		Events.FISHEYE_CMD, Events.CAMERA_MOTION_UPDATED, Events.CUBEMAP360_CMD, Events.ANTIALIASING_CMD,
		Events.BRIGHTNESS_CMD, Events.CONTRAST_CMD, Events.TOGGLE_STEREO_PROFILE_CMD,
		Events.TOGGLE_STEREOSCOPIC_CMD, Events.FPS_INFO);
	...
	Texture ldirt = new Texture(
		Gdx.files.internal(GlobalConf.scene.isHighQuality() ? "img/lensdirt.jpg" : "img/lensdirt_s.jpg"));
	...
	ppb.curvature.setEnabled(
		GlobalConf.program.STEREOSCOPIC_MODE && GlobalConf.program.STEREO_PROFILE == StereoProfile.VR_HEADSET);
	...
	case SCREENSHOT_SIZE_UDPATE:
		...
		if (pps[RenderType.screenshot.index] != null) {
		    if (changed(pps[RenderType.screenshot.index].pp, GlobalConf.screenshot.SCREENSHOT_WIDTH,
			    GlobalConf.screenshot.SCREENSHOT_HEIGHT)) {
			Gdx.app.postRunnable(new Runnable() {
			    @Override
			    public void run() {
				replace(RenderType.screenshot.index, GlobalConf.screenshot.SCREENSHOT_WIDTH,
					GlobalConf.screenshot.SCREENSHOT_HEIGHT);
			    }
			});
		    }
	...
	case FRAME_SIZE_UDPATE:
		...
		if (pps[RenderType.frame.index] != null) {
		    if (changed(pps[RenderType.frame.index].pp, GlobalConf.frame.RENDER_WIDTH,
			    GlobalConf.frame.RENDER_HEIGHT)) {
			Gdx.app.postRunnable(new Runnable() {

			    @Override
			    public void run() {
				replace(RenderType.frame.index, GlobalConf.frame.RENDER_WIDTH,
					GlobalConf.frame.RENDER_HEIGHT);
			    }
			});
		    }
		} else {
		    pps[RenderType.frame.index] = newPostProcessor(getWidth(RenderType.frame),
			    getHeight(RenderType.frame));
		}
	    break;
	case BLOOM_CMD:
	    Gdx.app.postRunnable(new Runnable() {
	...
	case CAMERA_MOTION_UPDATED:
		...
		public void run() {
		    ...
		    boolean cameraChanged = !Arrays.equals(cam.combined.val, prevCombined.val)
			    || !campos.equals(prevCampos);
			...
			if (pps[i] != null) {
			    ...
			    // Motion blur
			    ppb.motionblur
				    .setEnabled(GlobalConf.postprocess.POSTPROCESS_MOTION_BLUR != 0 && cameraChanged);
			}
		    }
	...
	case TOGGLE_STEREO_PROFILE_CMD:
	    boolean curvatureEnabled = GlobalConf.program.STEREOSCOPIC_MODE
		    && GlobalConf.program.STEREO_PROFILE == StereoProfile.VR_HEADSET;
	    boolean viewportHalved = GlobalConf.program.STEREOSCOPIC_MODE
		    && GlobalConf.program.STEREO_PROFILE != StereoProfile.ANAGLYPHIC
		...
		if (pps[i] != null) {
		    ...
		    ppb.lglow.setViewportSize(getWidth(currentRenderType) / (viewportHalved ? 2 : 1),
			    getHeight(currentRenderType));
		}
	    }
